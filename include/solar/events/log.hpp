...
struct Summary
{
    ...
    {
        const auto size =
            std::snprintf(output.data(), output.size(), "event %u occurrence %llu count %u",
...
template <typename EventT, typename SourceT, solar::log::Level LogLevel,
          typename Renderer = Summary>
struct On
{
    ...
    [[nodiscard]] static Result<void> process(RecordView record) noexcept
    {
        ...
        if (!rendered) {
            return fail<solar::Error>(rendered.error());
        }
        auto captured =
            solar::log::detail::emit<SourceT, solar::log::domain::Unclassified, LogLevel,
                                     solar::log::Operation::TryCapture, DefaultApplication,
                                     solar::log::Origin::Event>({}, "{}", *rendered);
        return captured ? Result<void>{}
                        : Result<void>{
                              fail<solar::Error>({.status = solar::status_of(captured.error())})};
    }
};
...
struct solar::events::event_log_extensions<Component, std::void_t<typename Component::EventLogs>>
{
