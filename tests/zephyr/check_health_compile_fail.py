...
CASES = {
    1: "SOLAR_DIAGNOSTIC_HEALTH_ASSESS_RETURN",
    2: "SOLAR_DIAGNOSTIC_HEALTH_CHECK_RETURN",
    3: "SOLAR_DIAGNOSTIC_HEALTH_PROGRESS_NOT_DECLARED",
    4: "SOLAR_DIAGNOSTIC_HEALTH_STACK_MARGIN_EXCEEDS_STACK",
}


def source_entry(path: pathlib.Path) -> dict:
    database = json.loads(path.resolve().read_text())
    return next(item for item in database if item["file"].endswith("health_compile_fail/src/main.cpp"))
