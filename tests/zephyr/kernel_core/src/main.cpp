...
{
    zassert_equal(held_mutex.lock(), solar::Status::Ok);
    holder_ready.give();
    zassert_equal(holder_release.take(), solar::Status::Ok);
    zassert_equal(held_mutex.unlock(), solar::Status::Ok);
}
...
void exercise_isr(const void* argument)
{
    ...
    context.queue_status = context.queue->try_send_isr(42);
    (void)context.events->post_isr(0x1);
    context.mutex_status = context.mutex->try_lock();
}
...
ZTEST(solar_kernel_core, test_priority_scheduler_and_current_thread)
{
    ...
    zassert_equal(kernel::this_thread::yield(), solar::Status::Ok);
    zassert_equal(kernel::this_thread::busy_wait_for(10us), solar::Status::Ok);
...
{
    kernel::Mutex mutex;
    zassert_equal(mutex.lock(), solar::Status::Ok);
    zassert_equal(mutex.try_lock(), solar::Status::Deadlock);
    zassert_equal(mutex.unlock(), solar::Status::Ok);
    zassert_equal(mutex.unlock(), solar::Status::PermissionDenied);
    ...
    zassert_equal(mutex.try_lock(), solar::Status::Ok);
    zassert_equal(mutex.unlock(), solar::Status::Ok);

    auto unique = kernel::UniqueLock<kernel::Mutex>::acquire(mutex);
    zassert_true(unique.has_value());
    zassert_true(unique->owns_lock());
    zassert_equal(unique->unlock(), solar::Status::Ok);
    zassert_false(unique->owns_lock());
    zassert_equal(unique->try_lock(), solar::Status::Ok);
    zassert_true(unique->owns_lock());
    zassert_equal(unique->unlock(), solar::Status::Ok);

    kernel::RecursiveMutex recursive;
    zassert_equal(recursive.lock(), solar::Status::Ok);
    zassert_equal(recursive.lock(), solar::Status::Ok);
    zassert_equal(recursive.unlock(), solar::Status::Ok);
    zassert_equal(recursive.unlock(), solar::Status::Ok);
    ...
    zassert_equal(holder_ready.take(kernel::Timeout::after(100ms)), solar::Status::Ok);
    zassert_equal(held_mutex.lock(kernel::Timeout::after(10ms)), solar::Status::Timeout);
    holder_release.give();
...
ZTEST(solar_kernel_core, test_semaphore_message_queue_and_events)
{
    ...
    zassert_equal(semaphore.try_take(), solar::Status::WouldBlock);
    semaphore.give();
    zassert_equal(semaphore.count(), 1);
    zassert_equal(semaphore.take(), solar::Status::Ok);
    zassert_equal(semaphore.take(kernel::Timeout::after(2ms)), solar::Status::Timeout);

    kernel::MessageQueue<std::uint32_t, 2> queue;
    const auto empty = queue.try_receive();
    zassert_false(empty.has_value());
    zassert_equal(empty.error(), solar::Status::Empty);
    zassert_equal(queue.try_send(1), solar::Status::Ok);
    zassert_equal(queue.try_send(2), solar::Status::Ok);
    zassert_true(queue.full());
    zassert_equal(queue.try_send(3), solar::Status::Full);
    zassert_equal(*queue.peek(), 1);
    zassert_equal(*queue.peek_at(1), 2);
    zassert_equal(*queue.try_receive(), 1);
    zassert_equal(queue.try_send_front(9), solar::Status::Ok);
    zassert_equal(*queue.try_receive(), 9);
    zassert_equal(*queue.try_receive(), 2);
    zassert_equal(queue.receive(kernel::Timeout::after(2ms)).error(), solar::Status::Timeout);
    queue.purge();
    zassert_true(queue.empty());

    kernel::EventFlags events;
    zassert_equal(events.try_wait_any_isr(0x1).error(), solar::Status::WouldBlock);
    ...
    zassert_equal(events.wait_any(0x1, kernel::Timeout::after(2ms)).error(),
                  solar::Status::Timeout);
    zassert_equal(events.wait_any(0, kernel::Timeout::no_wait()).error(), solar::Status::Invalid);
}
...
ZTEST(solar_kernel_core, test_poll_signal_semaphore_and_message_queue)
{
    ...
    zassert_equal(poll.add(signal, 1), solar::Status::Ok);
    zassert_equal(poll.add(semaphore, 2), solar::Status::Ok);
    zassert_equal(poll.add(queue, 3), solar::Status::Ok);
    zassert_equal(poll.add(signal, 4), solar::Status::Full);
    zassert_equal(poll.try_wait().error(), solar::Status::WouldBlock);
    ...
    zassert_equal(signal.raise(77), solar::Status::Ok);
    ...
    zassert_equal(queue.try_send(5), solar::Status::Ok);
    ...
    zassert_equal(poll.wait(kernel::Timeout::after(2ms)).error(), solar::Status::Timeout);
    zassert_equal(poll.event(9).error(), solar::Status::NotFound);
}
...
ZTEST(solar_kernel_core, test_timer_callback_context_and_sync)
{
    ...
    zassert_equal(timer.start_after(5ms), solar::Status::Ok);
    zassert_true(timer.running());
    zassert_equal(timer_signal.take(kernel::Timeout::after(100ms)), solar::Status::Ok);
    ...
    zassert_equal(timer.start_periodic(2ms, 2ms), solar::Status::Ok);
    zassert_equal(timer_signal.take(kernel::Timeout::after(100ms)), solar::Status::Ok);
    zassert_equal(timer_signal.take(kernel::Timeout::after(100ms)), solar::Status::Ok);
    timer.stop();
    ...
    zassert_equal(synchronized.start_after(2ms), solar::Status::Ok);
    const auto count = synchronized.sync();
    zassert_true(count.has_value());
    zassert_true(*count >= 1);
    zassert_equal(synchronized.start(kernel::Timeout::forever()), solar::Status::Invalid);
}
...
ZTEST(solar_kernel_core, test_isr_specific_operations)
{
    ...
    zassert_equal(context.queue_status, solar::Status::Ok);
    zassert_equal(context.mutex_status, solar::Status::Invalid);
