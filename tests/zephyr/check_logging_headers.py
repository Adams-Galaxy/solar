...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile every public Solar Logging header as an isolated Zephyr C++ TU."
    )
    ...
    headers = sorted((include_root / "solar" / "log").glob("*.hpp"))
    headers.append(include_root / "solar" / "log.hpp")
    failures = 0
