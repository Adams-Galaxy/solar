...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile every public Solar lifecycle header as an isolated Zephyr C++ TU."
    )
    ...
    database = json.loads(compile_commands.read_text())
    entry = next(
        (candidate for candidate in database if candidate["file"].endswith((".cpp", ".cxx"))),
        None,
    )
    if entry is None:
        print("no C++ compile command found", file=sys.stderr)
        return 2

    headers = sorted((include_root / "solar" / "lifecycle").glob("*.hpp"))
    headers.extend(
        [include_root / "solar" / "lifecycle.hpp", include_root / "solar" / "system" / "query.hpp"]
    )
    failures = 0
