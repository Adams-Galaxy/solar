...
def syntax_command(command: str, header: pathlib.Path) -> list[str]:
    ...
    if not source_removed:
        raise ValueError("compile command did not contain a C++ source")
...
def main() -> int:
    ...
    entry = next((item for item in database if item["file"].endswith((".cpp", ".cxx"))), None)
    if entry is None:
        print("no C++ compile command found")
        return 2
    root = args.include_root.resolve()
    headers = sorted((root / "solar" / "remote").glob("*.hpp")) + [root / "solar" / "remote.hpp"]
    failures = 0
    for header in headers:
        completed = subprocess.run(syntax_command(entry["command"], header), cwd=entry["directory"],
