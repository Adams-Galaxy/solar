...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile every public Solar Parameters header as an isolated Zephyr C++ TU."
    )
    ...
    headers = sorted((include_root / "solar" / "parameters").glob("*.hpp"))
    headers.append(include_root / "solar" / "parameters.hpp")
    failures = 0
