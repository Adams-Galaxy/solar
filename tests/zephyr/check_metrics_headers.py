...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile every public Solar Metrics header as an isolated Zephyr C++ TU."
    )
    ...
    headers = sorted((include_root / "solar" / "metrics").glob("*.hpp"))
    headers.append(include_root / "solar" / "metrics.hpp")
    failures = 0
