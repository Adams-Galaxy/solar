...
def syntax_command(command: str, header: pathlib.Path) -> list[str]:
    ...
    filtered.extend(["-Werror", "-fsyntax-only", "-include", str(header),
                     "-x", "c++", "/dev/null"])
    return filtered
...
def main() -> int:
    ...
    entry = next(item for item in database if item["file"].endswith("supervisor/src/main.cpp"))
    root = args.include_root.resolve()
    headers = sorted((root / "solar" / "supervisor").glob("*.hpp")) + [
        root / "solar" / "supervisor.hpp"]
    failures = 0
