...
CASES = {
    1: "SOLAR_DIAGNOSTIC_INVALID_INSPECTION_COLLECTION",
    2: "SOLAR_DIAGNOSTIC_INSPECTION_PROVIDER_MISSING",
    3: "SOLAR_DIAGNOSTIC_INSPECTION_COLLECTION_NOT_REGISTERED",
    4: "SOLAR_DIAGNOSTIC_INSPECTION_COLLECTION_CEILING",
}


def source_entry(path: pathlib.Path) -> dict:
    database = json.loads(path.resolve().read_text())
    return next(item for item in database if item["file"].endswith("inspection_compile_fail/src/main.cpp"))
